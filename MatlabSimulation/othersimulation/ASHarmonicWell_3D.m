...
Nbands=8;
ax=5.0; dx=0.4; [xvals,Nx] = GetASGridDx(ax,dx);
...
disp('Harmonic oscillator energies')
vals

%Unpack states from the parity-adapted representation
%Note that once we have unpacked the states all other operations are the same between the parity adapted and non-adpated codes
G_gs = UnpackAS3DState(vecs(1,:),Nx,Ny,Nz,'p','p')/sqrt(dx*dy*dz);
G_es = UnpackAS3DState(vecs(2,:),Nx,Ny,Nz,'p','p')/sqrt(dx*dy*dz);
G_gs2 = UnpackAS3DState(vecs(3,:),Nx,Ny,Nz,'p','p')/sqrt(dx*dy*dz);
G_es2 = UnpackAS3DState(vecs(4,:),Nx,Ny,Nz,'p','p')/sqrt(dx*dy*dz);
